...
2. [Document-First Scaffolding](#document-first-scaffolding)
3. [Multi-Stack Workspaces](#multi-stack-workspaces)
4. [Importing Existing Documents](#importing-existing-documents)
5. [Architecture Overview](#architecture-overview)
6. [Multi-Agent Architecture](#multi-agent-architecture)
7. [Complete Skill Reference](#complete-skill-reference)
8. [Sample Project Walkthrough](#sample-project-walkthrough)
9. [Agent Teams Walkthrough](#agent-teams-walkthrough)
...
- **Stack-specific**: Node.js 18+ (if Angular/React), Java 17+ and Maven 3.9+ (if Spring Boot), Go 1.21+ (if Go), etc.

### Create a New Project (Document-First)
...
#   1. Do you have existing documents? (product vision, PRD, architecture guide, etc.)
#   2. If yes → analyzes them → auto-detects tech stack, phases, workspaces
#   3. Confirms detected tech stack (you approve/change)
#   4. Asks for additional workspace tech stacks
#   5. Generates project with standard SDLC docs from imports
...
If you imported documents, `/sdlc init` will:
- Read imported docs and generate standard-format SDLC documents
- Auto-advance phases that are fully covered by imports
- Flag gaps in partially covered phases
- Start you at the first incomplete phase
...
## Document-First Scaffolding

The MultiStack kit uses a **document-first** approach: provide your existing documents before selecting tech stacks. The kit reads, analyzes, and auto-detects everything it can.

### What the Kit Detects from Documents
- **Tech stack**: Frameworks, languages, databases, messaging, infrastructure
- **Phase coverage**: Which SDLC phases are already covered (ideation, requirements, architecture, tech specs, backlog)
- **Workspaces**: Distinct system components (control plane, data plane, admin UI, etc.)

### Example: SASE Platform with Existing Implementation Guide
```bash
bash create-project.sh sase-platform ~/projects

# Step 1: Project name → sase-platform
# Step 2: Documents → /path/to/SASE-Master-Implementation-Guide.docx
#          Analysis: Angular 17+, Spring Boot 3.3, Go, Envoy, PostgreSQL, Kafka detected
#          Phases: ideation ✅, architecture ✅, techSpecs ✅
# Step 3: Confirm tech stack → y
# Step 4: Additional workspaces → Go Data Plane, Envoy Edge Proxy
# Step 5: Generated! Start at Phase 2 (Requirements — needs gaps filled)
```
...
Projects can include multiple tech stacks, each in its own workspace directory:

```
my-project/
├── frontend/                 # Primary frontend (Angular, React, Vue, etc.)
├── backend/                  # Primary backend (Spring Boot, Go, Node.js, etc.)
├── workspaces/               # Additional tech stacks
│   ├── go-data-plane/        # Go services
│   └── envoy-edge/           # Envoy proxy config
├── docs/tech-refs/           # Reference docs for all stacks
├── docs/tech-specs/shared-schemas/  # Cross-workspace contracts
└── .claude/rules/06-tech-stack-context.md  # Auto-generated stack context
```

### Key Files
- **`.sdlc/state.json → techStack`**: Machine-readable tech stack config (primary + additional)
- **`.claude/rules/06-tech-stack-context.md`**: Auto-generated rule file — single source of truth for all agents
- **`docs/tech-refs/`**: Reference documentation for all stacks

### Developing in Workspaces
```
/develop backend          # Implements in backend/ (primary backend)
/develop frontend         # Implements in frontend/ (primary frontend)
/develop workspace go-data-plane  # Implements in workspaces/go-data-plane/
```

### Regenerating Tech Context
After manually changing `state.json`, regenerate the context rule:
```bash
...
### At Scaffold Time
The `create-project.sh` scaffold asks for documents first. Supported formats:
- `.docx` — auto-extracted to markdown via python3
- `.pdf` — extracted via PyPDF2 or pdfplumber (install: `pip3 install PyPDF2`)
- `.md` / `.txt` — copied as-is

### How Documents Are Processed
1. **Copied** to `docs/tech-refs/` (original preserved)
2. **Extracted** to markdown (e.g., `docs/tech-refs/guide-extracted.md`)
3. **Analyzed** for tech stack, phases, and workspaces
4. **Used by `/sdlc init`** to generate standard SDLC documents and auto-advance phases

### Phase Auto-Advance
When `/sdlc init` finds imported documents:
- **Fully covered phases** → marked `completed` (source: imported)
- **Partially covered phases** → marked `in_progress` (source: imported_partial)
- Skills like `/enterprise-prd` will read the imported doc and fill gaps rather than starting from scratch
